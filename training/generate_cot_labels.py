...
def load_existing_results(output_file: str) -> Tuple[List[Dict[str, Any]], Set[str]]:
...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.batch_size) as executor:
        ...
        for future in tqdm.tqdm(concurrent.futures.as_completed(future_to_item), total=len(input_data)):
